...
def _contains_analyze(_value: JsonValue, _ctx: AnalyzeContext) -> StaticFacts:
    return StaticFacts(
        subschemas=((),),
        produces=(CONTAINS_ID,),
        evaluates_indexes=DynamicIndexes(),
    )


def _min_contains(ctx: KeywordContext) -> int | float:
    value = ctx.schema.get("minContains")
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        return 1
    return value


def _max_contains(ctx: KeywordContext) -> int | float | None:
    value = ctx.schema.get("maxContains")
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        return None
    return value


def _contains_evaluate(_value: JsonValue, cursor: Cursor, ctx: KeywordContext) -> bool:
    instance = cursor.value
    if not isinstance(instance, list):
        return True
    matched: list[int] = []
    ...
    count = len(matched)
    minimum = _min_contains(ctx)
    maximum = _max_contains(ctx)
    if count < minimum or (maximum is not None and count > maximum):
        ...
        return False
    # Dependency data comes only from an accepting keyword: matched indexes,
    # or True when every element matched (minContains: 0 with zero matches
    # produces nothing, matching "an accepting keyword with nothing to
    # report" — every other producer in this module follows the same rule).
    if count > 0:
        ctx.produce(True if count == len(instance) else matched)
    return True


CONTAINS = KeywordBehavior(
    id=CONTAINS_ID, evaluate=_contains_evaluate, analyze=_contains_analyze
)
