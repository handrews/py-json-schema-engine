...
- `UnknownDialectError.dialect_uri`: the dialect an *embedded* resource
  demanded mid-walk (P14), set so the engine can assemble it and register
  again. `None` for a document-root `$schema`, which is resolved before the
  walk begins.
