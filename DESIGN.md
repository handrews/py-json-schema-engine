...
| P11 | Embedded-resource location chain | **Direction agreed, design pending (§7).** A canonical `base_uri#pointer` does not locate an error inside a compound document: a reader who never saw the internal `$id` structure cannot tell which embedded resource `https://x.example/inner#/properties/a` lives in, nor where that resource sits in the file they registered. The intent is a **chain** of locations — the erroring position first, then each enclosing `$id`-bearing resource with the pointer to it within its parent, up to the root `$id`, retrieval URI, or however the caller named the document. The registry already carries the raw material: `_walk` holds the enclosing `base_uri`/`pointer` in scope at the moment an embedded `$id` reassigns them, and `DocumentLocation` already records a (document, document-rooted pointer) pair — deliberately flat, since D17 needs only the outermost document to find a source range. | The bundled-schema shape (many `$id` resources under one document's `$defs`) is the common real one, and it is exactly where a single canonical URI stops being actionable — most sharply for `UnresolvableReferenceError`, where the base a reference resolved against is itself an embedded `$id`. | Settled when §7's open questions are answered. |
...
7. **Location chain for embedded resources** (P11) — direction agreed, four
   questions open. (a) Is the chain carried on the exception or derived on
   request? Raise sites in `evaluator`, `regex`, and keyword `facts()` hold a
   `base_uri#pointer` and no registry handle, so building it at construction
   means threading the registry everywhere; deriving it keeps D17's
   "zero cost unless asked" stance. (b) Does it subsume `Engine.locate()`?
   The chain's last hop is that method's answer, and shipping two overlapping
   location services would be a mistake. (c) How does `str(error)` render a
   chain without making the common single-resource message noisy? (d) Does
   `UnresolvableReferenceError` also gain the reference string and the base it
   resolved against — the chain says where you are, not what resolution was
   attempted. Out of scope either way: no spec output format has a field for
   this, so it stays a side API like D17 positions, never an output unit.
8. **Atomic registration** — registration is not atomic, and nothing has ever
