...
| P12 | Duplicate identifiers are errors | Two different schemas may not claim one resource URI, and two different schema objects may not claim one anchor name within a resource. `DuplicateResourceError` covers a single document minting an `$id` twice and a later registration rebinding a URI an earlier one bound to a *different* schema (`json_equal`, so re-registering an equal document stays a no-op). `DuplicateAnchorError` covers `$anchor`, `$dynamicAnchor`, and the draft-07/06 `$id: "#name"` form alike, keyed on the `_anchors` entry that all three write; one object carrying both anchor kinds under one name claims the same key with the same target and is allowed. Claims are scoped to one registration walk, so a re-registered resource rewrites its own anchors without a false positive, and a `$ref` sibling under draft-07/06 claims nothing because it suppresses identifiers (D18). | Both were silent last-write-wins, so one schema shadowed the other and a reference resolved to whichever the walk reached last. The anchor case was worse than shadowing: because a dynamic anchor is also a plain anchor (D8), `$anchor: "n"` on one object and `$dynamicAnchor: "n"` on another left `$ref` and `$dynamicRef` resolving the same fragment to *different* schemas. The spec is silent on duplicate `$id` and deems a duplicate anchor undefined behavior an implementation MAY reject; the owner deems rejection correct for both. Duplicate `$id` is also what made the P11 parent relation cyclic. | A schema in the wild proves a duplicate is load-bearing somewhere (none known; the vendored suite, the bundled metaschemas, and every repo fixture are clean). Extended by P15: aliases and bundled URIs are claims, and `unregister` is the replacement path. |
| P13 | Registration is all-or-nothing | `SchemaRegistry.register` journals every index write and undoes them if anything escapes, so a document that fails leaves the registry exactly as it found it. A `_Registration` holds `(index, key, prior)` for the eight dict indexes (a `_pop` removal is recorded the same way) and `(index, member)` for the four set ones, recording only genuinely new members because `_produced_ids`/`_consumed_ids` are unions. The undo restores prior values rather than deleting keys: a walk legitimately rebinds entries an earlier registration owns, and `_documents` order is what `resources()` promises. `except BaseException`, since `_step` raises bare `KeyError`/`TypeError`, `RecursionError` can fire anywhere, and caller code runs inside the walk. Not journaled: `_reference_memo` (a derived cache, cleared on both paths), the P12 claim sets (per-walk scratch), and the regex cache (keyed by pattern, a function of a fixed dialect and backend). On the way out the registry fills the error's `location_chain` (P11) and `schema_source` (D17) *before* the undo — the only moment the indexes they derive from still exist — and `_canonical` refuses to start a lazy bundled registration while a journal is live. | The half-indexed document left behind was still evaluable, and its partial `produces`/`consumes` fed D5's elision predicate, so the failure surfaced as a wrong answer rather than a loud one. P12 added two more ways to fail mid-walk. Journaling rather than snapshotting keeps the cost O(this document's writes) with no term in registry size, which matters because every lookup may lazily register a bundled metaschema; measured, registration is unchanged (1.27 ms either way on the OAS 3.1 corpus). | A caller needs a *successful* registration undone: answered by P15's `unregister`, which resources go with their current owner. |
| P14 | Per-resource dialects | `$schema` governs the schema resource it roots, not the document (2020-12 core §8.1.1). `_walk` rebinds its `dialect` local wherever it rebinds `base_uri`, so the inner dialect supplies the keyword table, `ref_ignores_siblings`, and the identifiers minted into that resource, and `_document_dialects` records it. **The boundary is decided from the outside, the contents from the inside:** `base_id` comes from the *parent* extractor — its `$id` syntax is what decides a resource starts here at all, and a relative `$schema` has no base to resolve against until it has — while `anchors`, `dynamic_anchor` and `recursive_anchor` are re-read with the inner one. Honored under every dialect; draft-07/06 are silent on the placement rather than prohibitive. A `$schema` where no resource starts is **ignored**, not refused: the spec forbids the placement, but refusing it is strict-mode hygiene, which D14 keeps opt-in, and `{"$schema": X, "not": {"$schema": X}}` is how Bowtie spells "allows nothing" for every dialect it tests. Only the walk can discover an embedded `$schema`, and the registry holds no loaders, so it raises `UnknownDialectError` carrying `dialect_uri` and the engine assembles that dialect and registers again; P13 rollback is what makes the retry start from the state the first attempt found. Pointer navigation in `resolve_ref`/`child` refreshes its extractor at each base change, and since P16 it changes base only where the walk created a resource, so the dialect it looks up always exists. | The walk threaded the document root's dialect through the whole recursion, so the bug cut both ways: a valid draft-07 resource embedded in a 2020-12 document was rejected for array-form `items`, and a 2020-12 resource embedded in a draft-07 document had `$id: "#foo"` accepted as an anchor. It also mislabeled any node reached by a pointer that crossed an `$id` under the wrong extractor as its resource's root. Evaluation and the compiler already keyed every dialect lookup on the unit's own base URI, so the walk was the only tier disagreeing — with them and with itself. | A dialect needs to *forbid* an embedded `$schema`: that wants a `Dialect` field beside `identifiers` and `ref_ignores_siblings`. A lint layer under D14 wants to flag a misplaced `$schema`, which this deliberately does not. |
| P15 | Explicit unregister; aliases and bundled URIs are claims | P12's one-resource-one-schema rule extends to the two other ways a URI can be bound. A **retrieval alias** is consulted before the resource index, so an `$id` equal to an existing alias, or a retrieval URI that already names a resource or aliases a different one, would leave one of them unreachable; each is a `DuplicateResourceError` (`_claim_resource`, `_check_alias`). A **bundled metaschema's URI** is reserved (owner, 2026-09-23): it may be claimed only with content `json_equal` to the bundled document, checked against `_bundled` when it has not been lazily registered yet, so the answer no longer depends on whether the metaschema has been used. A custom metaschema goes under its own URI, as custom dialects already do. Matches the TypeScript engine's ADR 0005, which registers its metaschemas eagerly and so reaches the same answer without the `_bundled` check. **Replacement is explicit** (the ADR 0005 shape): `SchemaRegistry.unregister(uri)` / `Engine.unregister_schema(uri)`, no `replace=`. `uri` must name a document root, directly or by retrieval URI; anything else is `UnresolvableReferenceError`, naming the owning document for an embedded resource. `_owned_resources` records each document root's P12 claim set, and `_resource_locations` stays the truth for who owns a resource *now*: an equal embedded copy takes a resource over, and a document whose root is taken over that way stops being one. Unregister drops what the document still owns, and every alias pointing at it. The produced/consumed unions and the pending queue are left alone. It is refused on a snapshot, during a registration (caller code in a walk, which the rollback would undo), and for a bundled URI. **One divergence from TypeScript (owner, 2026-09-23):** an equal re-registration keeps every retrieval alias, where TypeScript's evicts all but the newest. A document reachable from two URLs would otherwise flip-flop, breaking one set of `$ref`s each time it is loaded through the other. This is filed on the TypeScript backlog. | After P12, three shapes still shadowed silently: an `$id` equal to a retrieval alias, a reused retrieval URI, and a bundled URI whose answer depended on whether the metaschema had been used. And a modified document could not be replaced without a fresh engine, which also discarded every other registration. | A caller needs to override a bundled metaschema in place, or the TypeScript engine settles its D16 alias question differently. |
| P16 | Navigation follows registration | An `$id` identifies only in a schema position (§5), and only the registration walk knows which positions those are — navigation cannot tell without re-running every keyword's `analyze()`. So `resolve_ref`'s pointer walk and `child` ask what the walk concluded: at an object whose `$id` the enclosing dialect reads as a base, they rebase only when the registry holds a resource at that URI and the node is that resource (`_entered`). Identity is the common case, since `_claim_resource` stores the walked node. `json_equal` is the fallback for a resource an equal copy elsewhere has taken over (P15), whose node here is a different object with the same content. An `$id` in `enum`, `const`, `examples`, an unknown keyword, or a draft-07 `$ref` sibling is data, and the pointer carries on under the enclosing resource. Refusing pointers that leave schema positions was rejected: the suite's `optional/refOfUnknownKeyword.json` follows `$ref`s into unknown keywords and `examples`. Rebasing onto any *known* URI was rejected: `optional/unknownKeyword.json` writes a real resource's URI into data. The check runs only where an `$id` is present, so `child`'s hot path costs what it did. | Navigation rebased at every `$id` it stepped onto. A `$ref` into `enum`, `examples` or unknown-keyword data raised `unknown schema` for a URI that was never an identifier, or resolved relative references against an invented base. | The `json_equal` fallback shows up in a profile, for a taken-over copy that is navigated on a hot path. That calls for memoizing by node identity. |
| P17 | Per-resource metaschema validation | `validate_schemas` checks each **dialect region** against its own metaschema, following 2020-12 core §9.3.3: a compound document SHOULD NOT be validated by applying one metaschema to the whole of it, and each resource SHOULD be validated against its own. A region is the root, plus every embedded resource whose dialect differs from its parent's. A same-dialect resource stays in its parent's region, where the same metaschema reaches it, so nothing is checked twice. Each region's node is evaluated with the regions nested inside it masked to `{}` (path-copied; `{}` is a valid schema at every schema position). Without masking, the outer metaschema judged the inner dialect: a valid draft-07 resource under 2020-12 was refused, and an invalid 2020-12 resource under draft-07 registered. The check still runs before registration. Only the walk knows the resources and their dialects, so `SchemaRegistry.survey` runs it as a dry run through P13's transaction, rolled back unconditionally, capturing each resource's chain and source first. The engine's P14 retry loop assembles embedded dialects for the survey exactly as for a registration. A region whose metaschema is unavailable is skipped, as the root always was (owner, 2026-09-23). If the survey itself refuses the document, the root is checked against its metaschema first, so a document broken both ways still reports `SchemaValidationError`. Cost: a validated registration walks twice; the option is off by default. | The root's metaschema judged every embedded resource as its own dialect. | A metaschema needs to see an embedded resource in context, e.g. a custom metaschema constraining where a nested dialect may appear. Masking hides exactly that. |
...
7. **Staged registration as a behavior-neutral refactor of P13.** The
    ...
    touching, not as work owed on its own. `unregister` (P15) did not need
    it: removal runs outside any registration, and the journal only gained
    an undoable pop. P11's `_describe` and D17's `schema_source`
...
- "No way to replace a registered document": `Engine.unregister_schema`,
  in the TypeScript engine's ADR 0005 shape (P15). Unregister then register
  replaces a document.
- "Pointer navigation reads identifiers in non-schema positions": P16.
  Navigation rebases only where the registry holds a resource and the node
  is that resource, by identity or as an equal copy it took over.
- "Per-resource metaschema validation": P17. Each dialect region is
  checked against its own metaschema with nested regions masked, found by a
  registration dry run (`survey`). An unavailable metaschema is skipped, at
  every level.
