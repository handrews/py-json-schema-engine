# The official draft6 `optional/format` leg (DESIGN.md M7): every format
...
from json_schema_engine.formats import FORMATS_DRAFT_06
from json_schema_engine.test_kit import SuiteCase, collect_suite_params, count_params

from .test_draft6 import REMOTES_DIR, RETRIEVAL_URI, SUITE_DIR, UNSUPPORTED
...
EXPECTED_RUN, EXPECTED_SKIPPED = 407, 0


@pytest.mark.parametrize("case", PARAMS)
def test_format_case(case: SuiteCase) -> None:
    formats_case(case, DIALECT_DRAFT_06, FORMATS_DRAFT_06, REMOTES_DIR, RETRIEVAL_URI)
