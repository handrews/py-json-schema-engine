...
def u0(v, d, s, st, pn, cu, ev):
    ...
    if u2(v, d, s, st, H_PATH(pn, 'anyOf/1'), cu, ev):
        ...
        del ev[m5:]
    if not c0:
        H_ERR(st, x2, pn, cu, 'does not match any anyOf branch', None)
        w0 = False
    if w0:
        H_DROP(st, m1)
    m6 = H_EMARK(st)
    if g0:
        ...
        for b3 in v:
            if b3 not in b1:
                b2.append(b3)
                m7 = H_AMARK(st)
                if not H_FALSE(st, x4, H_PATH(pn, 'unevaluatedProperties'), H_CHILD(cu, b3, v[b3])):
                    w1 = False
                    H_ACUT(st, m7)
        if w1:
